  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        {activePeriod ? (
          <p className="text-sm text-gray-500">Active period: <span className="font-medium text-accent">{activePeriod.name}</span></p>
        ) : (
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm text-center">
          <p className="text-2xl font-bold text-accent">{myClasses.length}</p>
      ...
      {myClasses.map(({ class: cls, isHomeroom }) => {
        ...
        return (
          <div key={cls.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-4">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <a href="/teacher/gradebook" className="text-xs text-accent hover:underline">Open Gradebook →</a>
            </div>
              ...
              {students.slice(0, 5).map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center gap-3 px-5 py-3">
                    ...
                    <span className="flex-1 text-sm font-medium text-ink">{s.fullName}</span>
                    {avg !== null ? (
      ...
      {myDomains.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mt-4">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-ink">My Domain Assignments</h2>
          ...
          <div className="divide-y divide-gray-50">
            {myDomains.map((da) => (
              <div key={da.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <span className="text-sm text-ink">{da.competencyArea.name}</span>
